...
function InterventionsPage() {
  ...
  const columns: Column<Intervention>[] = [
    { key: "reference", label: "Référence", render: (i) => <span className="font-medium text-slate-800">{i.reference}</span> },
    { key: "date", label: "Date", render: (i) => <span className="text-slate-500 text-xs">{i.date}</span> },
    { key: "luminaireId", label: "Luminaire", render: (i) => <span className="text-slate-700">{i.luminaireId}</span> },
    { key: "type", label: "Type", render: (i) => <span className="text-slate-600">{i.type}</span> },
    { key: "technicien", label: "Technicien", render: (i) => <span className="text-slate-700">{techNom(i.technicienId)}</span> },
    { key: "duree", label: "Durée", align: "right", render: (i) => <span className="text-slate-600">{i.duree_min} min</span> },
    { key: "statut", label: "Statut", render: (i) => <StatusBadge label={i.statut === "TERMINEE" ? "Terminée" : "En cours"} tone={i.statut === "TERMINEE" ? "green" : "orange"} /> },
  ];

  return (
    <div className="space-y-5">
      <ModuleHeader title="Suivi des interventions électriques" subtitle={`${filtered.length} interventions · matériel, durée, technicien`} />
