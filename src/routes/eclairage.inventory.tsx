...
function InventoryPage() {
  ...
  const columns: Column<Luminaire>[] = [
    { key: "reference", label: "Référence", render: (l) => <span className="font-medium text-slate-800">{l.reference}</span> },
    { key: "type", label: "Type", render: (l) => <StatusBadge label={l.type} tone={TYPE_COLOR[l.type] as any} /> },
    { key: "puissance", label: "Puissance", align: "right", render: (l) => <span>{l.puissance_w} W</span> },
    { key: "etat", label: "État", render: (l) => <StatusBadge label={l.etat} tone={ETAT_TONE[l.etat]} /> },
    { key: "secteur", label: "Secteur", render: (l) => <span className="text-slate-700">{zoneNom(l.site_id)}</span> },
    { key: "date_pose", label: "Date pose", render: (l) => <span className="text-slate-500 text-xs">{l.date_pose}</span> },
    {
      key: "carte",
      label: "Carte",
      align: "right",
      render: (l) => (
        <span onClick={(e) => e.stopPropagation()}>
          <Link to="/eclairage/map" search={{ focus: l.id }} className="inline-flex items-center gap-1 text-xs text-sky-600 hover:text-sky-800 font-medium">
  ...
  return (
    <div className="space-y-5">
      <ModuleHeader title="Inventaire des luminaires" subtitle={`${filtered.length} luminaires · vue tabulaire unifiée`} />
      ...
      {sel && (
        <DetailModal title={sel.reference} onClose={() => setSel(null)}>
          ...
          {sel.hauteur_feu_m && <DetailRow label="Hauteur de feu" value={`${sel.hauteur_feu_m} m`} />}
          {sel.temperature_couleur_k && <DetailRow label="Température" value={`${sel.temperature_couleur_k} K`} />}
          <DetailRow label="Secteur" value={zoneNom(sel.site_id)} />
          {sel.date_pose && <DetailRow label="Date de pose" value={sel.date_pose} />}
          <Link to="/eclairage/map" search={{ focus: sel.id }} className="mt-3 inline-flex items-center gap-1.5 text-sm text-white bg-amber-500 hover:bg-amber-600 rounded-lg px-3 py-2 font-medium">
