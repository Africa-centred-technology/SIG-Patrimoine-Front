    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-500">
          {filtered.length} luminaires · vue tabulaire unifiée
        </p>
      </div>
            ...
            {filtered.map((l) => (
              <tr key={l.id} className="hover:bg-amber-50/40">
                ...
                <td className="px-4 py-3">
                  <StatusBadge label={l.type} tone={TYPE_COLOR[l.type] as any} />
                </td>
                <td className="px-4 py-3 text-right">{l.puissance_w} W</td>
                <td className="px-4 py-3">
                  <StatusBadge label={l.etat} tone={ETAT_TONE[l.etat]} />
                </td>
                <td className="px-4 py-3 text-slate-700">
                  {ZONES.find((z) => z.id === l.site_id)?.nom}
                </td>
