...
function TeamsPage() {
  ...
  const filtered = TECHNICIENS.filter(
    (t) =>
      ...
      (query === "" ||
        ...
        t.habilitations.join(" ").toLowerCase().includes(query)),
  );
  ...
  const columns: Column<Technicien>[] = [
    ...
    },
    {
      key: "equipe",
      label: "Équipe",
      render: (t) => <span className="text-slate-600">{t.equipe}</span>,
    },
    {
      key: "habilitations",
      label: "Habilitations",
    ...
    },
    {
      ...
      label: "Interv./mois",
      align: "center",
      render: (t) => <span className="font-semibold text-slate-700">{t.interventionsMois}</span>,
    },
    {
      key: "tel",
      label: "Contact",
                ...
                .map((i) => (
                  ...
                  >
                    <span className="text-slate-700">
                      {i.reference} · {i.luminaireId}
                    </span>
                    <span className="text-slate-400">{i.date}</span>
                  </li>
                ))}
              {interventionsDe(sel.id).length === 0 && (
                <li className="text-xs text-slate-400">Aucune intervention enregistrée.</li>
              )}
            </ul>
          </div>
        </DetailModal>
